...
def identity_map_snapshot() -> dict[str, Any]:
    return {
        ...
        "relations_today": [
            ...
            {"from": "USER_HUMAN", "to": "NODE", "via": "R348 /identity/user-node (challenge+sig)", "cardinality": "CODED_SCAFFOLD"},
            {"from": "DEVICE_HOST", "to": "NODE", "via": "same process AppState", "cardinality": "1 host → 1 node process"},
        ],
        "target_relations": [
            {"from": "USER_HUMAN", "to": "NODE", "via": "R348 challenge+signature API", "cardinality": "CODED_SCAFFOLD"},
            {"from": "USER_HUMAN", "to": "WALLET", "via": "NOT WIRED at create (R349 next)", "cardinality": "MISSING"},
        ...
        "r348": {
            "user_node_association": "CODED_SCAFFOLD",
            "endpoints": [
