#!/usr/bin/env python3
"""R465 — ARTCB Lexicon Builder : téléchargement et extraction des dictionnaires complets.

Directive R464 : 100% des mots de chacune des 14 langues dans la bibliothèque
...
Usage :
    python3 scripts/artcb_r465_lexicon_build.py [--lang fr] [--all] [--test]

Mode --test : extrait les 2000 premiers lemmes par langue (validation de pipeline).
Mode --all  : télécharge et extrait l'intégralité de chaque langue.
...
# ── Configuration des 14 langues ──────────────────────────────────────────────
# Source : kaikki.org — pages dictionary par langue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="R465 — ARTCB Lexicon Builder — 14 langues depuis kaikki.org/Wiktionary"
    )
    parser.add_argument("--lang", help="Code ISO langue à builder (ex: fr, en, ar) — défaut: toutes")
    parser.add_argument("--all", action="store_true", help="Toutes les 14 langues (téléchargement complet)")
    ...
    # Sélection des langues à builder
    if args.lang:
        if args.lang not in LANG_CONFIG:
            logger.error("Langue inconnue : %s. Disponibles : %s", args.lang, list(LANG_CONFIG.keys()))
            sys.exit(1)
        lang_ids = [args.lang]
    elif args.all or not args.lang:
        lang_ids = list(LANG_CONFIG.keys())
    else:
        lang_ids = list(LANG_CONFIG.keys())
    ...
    for lang_id in lang_ids:
        ...
        try:
            ...
            logger.info(
                ...
                lang_id, result["entry_count"], result["status"],
            )
        except Exception as exc:
