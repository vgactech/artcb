#!/usr/bin/env python3
"""Provision (or reuse) OVH Public Cloud instance for ovh-node-2.

Project 1fc10a3fb27d4511a8c7873cd16243f2 (nic vc491276-ovh). GRA11 d2-8.
Credentials from Doppler artcb-2 / local ~/.artcb/nodes/ovh-node-2.env.
Never prints secrets. Never writes secrets into git. Does not touch OVH1.
"""
...
INSTANCE_NAME = "node-artcb-ovh-2"
SSH_KEY_NAME = "artcb-ovh-node-2"
...
def _creds() -> dict[str, str]:
    ...
    out = {
        "OVH_APPLICATION_KEY": local.get("OVH_APPLICATION_KEY") or os.environ.get("OVH_APPLICATION_KEY") or "",
        "OVH_APPLICATION_SECRET": local.get("OVH_APPLICATION_SECRET") or os.environ.get("OVH_APPLICATION_SECRET") or "",
        "OVH_CONSUMER_KEY": local.get("OVH_CONSUMER_KEY") or os.environ.get("OVH_CONSUMER_KEY") or "",
    }
    # Prefer Doppler artcb-2 over Cursor/shared OVH1 env.
...
def ovh(method: str, path: str, body: dict | None = None) -> tuple[int, Any]:
    ...
    if not (ak and as_ and ck):
        return 0, {"error": "missing_ovh2_creds"}
...
def ensure_ssh_key() -> dict[str, Any]:
    priv = Path.home() / ".ssh" / "artcb_ovh_node_2"
    pub = Path.home() / ".ssh" / "artcb_ovh_node_2.pub"
    Path.home().joinpath(".ssh").mkdir(mode=0o700, exist_ok=True)
    if not priv.is_file():
        proc = subprocess.run(
            ["ssh-keygen", "-t", "ed25519", "-f", str(priv), "-N", "", "-C", "artcb-ovh-node-2"],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        priv.chmod(0o600)
    repo_pub = ROOT / "deploy" / "artcb_ovh_node_2.pub"
    if pub.is_file():
    ...
    return {"ok": bool(kid), "ssh_key_id": kid, "created": True, "http": ccode, "body_keys": sorted((created or {}).keys()) if isinstance(created, dict) else []}


def _pick_flavor_image() -> tuple[str | None, str | None]:
    fcode, flavors = ovh("GET", f"/cloud/project/{PROJECT_ID}/flavor?region={REGION}")
    flavor_id = None
    if isinstance(flavors, list):
        for f in flavors:
            if f.get("name") == FLAVOR_NAME and f.get("available"):
                flavor_id = f.get("id")
                break
    icode, images = ovh("GET", f"/cloud/project/{PROJECT_ID}/image?region={REGION}&osType=linux")
    image_id = None
...
def existing_instance() -> dict[str, Any] | None:
    ...
    if code != 200 or not isinstance(inst, list):
        return None
    for it in inst:
        if it.get("name") in {INSTANCE_NAME, "node artcb ovh 2", "node artcb 2"}:
            return it
        tags = it.get("tags") or []
        if "aws-node-3" in str(tags):
            continue
        if it.get("id") and (it.get("name") or "").startswith("node-artcb-ovh-2"):
            return it
    return None
...
def wait_active(instance_id: str, timeout: int = 300) -> dict[str, Any]:
...
def launch(diag: dict[str, Any]) -> dict[str, Any]:
    ...
    if found and found.get("id"):
        ...
        diag.update({"launched": False, "reused": True, "instance_id": found.get("id"), "public_ip": ip, "status": waited.get("status") or found.get("status")})
        return diag
    key = ensure_ssh_key()
    if not key.get("ok"):
    ...
    if not iid:
        diag.update({"launched": False, "reason": "run_instance_failed", "http": code, "body_keys": sorted(created.keys()) if isinstance(created, dict) else [], "error": (created.get("message") or created.get("errorCode")) if isinstance(created, dict) else None})
        return diag
