#!/usr/bin/env bash
# Deploy ARTCB on OVH4 (node-artcb-ovh-4). Never touches OVH1 152.228.144.34 or OVH2 151.80.107.29.
# Usage: bash scripts/deploy_ovh4.sh IP BRANCH
set -Eeuo pipefail
SERVER_IP="${1:-}"
BRANCH="${2:-${ARTCB_DEPLOY_BRANCH:-}}"
SSH_USER="${OVH4_DEPLOY_USER:-ubuntu}"
SSH_KEY="${ARTCB_OVH4_SSH_KEY:-${HOME}/.ssh/artcb_ovh_node_4}"
REPO="$(cd "$(dirname "$0")/.." && pwd)"
KNOWN_HOSTS="${ARTCB_OVH4_KNOWN_HOSTS:-$REPO/deploy/ovh_artcb_node_4.known_hosts}"

if [ -z "$SERVER_IP" ] || [ -z "$BRANCH" ]; then
  echo "Usage: bash scripts/deploy_ovh4.sh IP BRANCH"
  exit 1
fi
if [ "$SERVER_IP" = "152.228.144.34" ]; then
  echo "Refusing to deploy OVH4 script onto OVH1"
  exit 1
fi
if [ "$SERVER_IP" = "151.80.107.29" ]; then
  echo "Refusing to deploy OVH4 script onto OVH2"
  exit 1
fi
if [ ! -f "$SSH_KEY" ]; then
...
echo "── Pose Doppler artcb-4 (token jamais affiché)"
DOPPLER_TMP="$(mktemp)"
...
token = (os.environ.get("KEY_API_ARTCB_DOPPLER_4") or local.get("KEY_API_ARTCB_DOPPLER_4") or local.get("DOPPLER_TOKEN") or "").strip()
if token:
    lines.append("DOPPLER_TOKEN=" + token)
lines.append("DOPPLER_PROJECT=artcb-4")
lines.append("DOPPLER_CONFIG=dev")
lines.append("ARTCB_NODE_ID=ovh-node-4")
...
echo "── Déploiement ARTCB ovh-node-4 → $SSH_USER@$SERVER_IP (branche $BRANCH)"
ssh "${SSH_OPTS[@]}" "$SSH_USER@$SERVER_IP" \
  env BRANCH="$BRANCH" SERVER_IP="$SERVER_IP" \
  bash -s <<'REMOTE'
set -Eeuo pipefail
export DEBIAN_FRONTEND=noninteractive
export ARTCB_NODE_ID=ovh-node-4
...
if [ -f /etc/artcb/doppler.env ]; then
  ...
  grep -q '^ARTCB_NODE_ID=' .env 2>/dev/null || echo 'ARTCB_NODE_ID=ovh-node-4' >> .env
...
if [ ! -f /etc/artcb/tls/server.crt ]; then
  sudo openssl req -x509 -newkey rsa:2048 -sha256 -days 825 -nodes \
    ...
    -subj "/CN=${SERVER_IP}/O=ARTCB/OU=ovh-node-4" \
...
echo "── OVH2 151.80.107.29 n'a PAS été modifié"
