...
remoteConsole/VNC URL (running instance, not rescue) and report it — it
does not type into the console automatically.
...
def _ovh4_console_url() -> dict[str, Any]:
    ...
    # remoteConsole on the RUNNING instance — never a rescue image.
    path = f"/cloud/project/{project}/instance/{instance}/remoteConsole"
    url = OVH_BASE + path
    body = json.dumps({"protocol": "vnc"})
    sig_input = "+".join([as_, ck, "POST", url, body, ts])
    ...
    req2 = Request(url, data=body.encode(), method="POST", headers=headers)
    try:
        ...
        return {"ok": False, "http": exc.code, "reason": "remoteConsole_failed"}


def main() -> int:
    # Load Doppler PEMs first (never printed).
    ...
    # Success if the three homogenized nodes SSH without rescue.
    need = {"ovh-node-1", "ovh-node-2", "aws-node-3"}
    got = {r["node_id"] for r in rows if r.get("ssh")}
    return 0 if need <= got else 2
