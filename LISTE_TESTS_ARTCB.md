...
| T-E55 | `ARTCB_INGEST_PROMPT_FILE` + `POST /ai/memo` OVH1 + mesure stack HTTP/nginx | Prompt user_query gravé caractère-par-caractère (max 32000) ; thinking/tokens **pas** on-chain ; Cursor transcript ≠ ARTCB ; stack mesurée vs InfiniBand/QUIC | [x] | 2026-09-08 SHA `230b8eb…` ×4 ; mémo **1095** `2f70dedf…` cert ×4 height **1096** ; 409 puis view 5→6 ; HTTP/1.1 nginx 1.24 ; pas IB/QUIC/WG ; rapports 267 |
