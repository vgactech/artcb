...
| T-E70 | `pytest tests/test_e2e286_mac_ssh.py` + live `scripts/run_live286_mac_ssh.py` | mac-node-local hors OFFICIAL_COMPUTE ; RFC1918 ; KEY_API_ARTCB_DOPPLER_MAC = secret Cursor env + artcb-blockchain/dev (prd seul = FAIL) ; jamais dump SSH ; timeout LAN ≠ PASS ; CERTIFIED_100=false | [ ] | 2026-09-09 probe cloud : token MAC absent de l’env Cursor ; artcb-blockchain/dev n=67 sans KEY_API_ARTCB_DOPPLER_MAC ; prd HTTP 400 (token dev) ; TCP 10.234.49.2:22/:8001 TimeoutError. Pas de SHA health Mac inventé. CERTIFIED_100=false. |
